...
CASES = [
    ...
    (
        ...
        "caution",
        {},
        "400/500 = 0.80. 명세 R1은 `ratio > 0.80`이므로 status는 등호에서 pass. 단 §4-2 비례 배점은 60%부터 가산하므로 R1 15×(0.80-0.60)/0.40 = 8점, R2-b 30×0.5 = 15점, 합 23 → 주의. status가 전부 pass여도 부담률 자체가 점수를 만든다.",
    ),
    (
        "r1-ratio-just-above",
        ...
        "405/500 = 0.81. 둘 다 fail이지만 §4-2 비례 배점으로 R1 15×(0.81-0.60)/0.40 = 8점, R2-b 30×0.525 = 16점, 합 24 → 주의. 계단 배점이던 시절 45점(위험)에서 내려온 값이다. 80.0%(23점)와 81%(24점)의 차이가 1점뿐이라 절벽이 사라졌다. 오버라이드는 0.90 초과가 아니라 미발동.",
    ),
    (
    ...
    (
        ...
        "caution",
        {"mortgage_ratio": "fail"},
        "근저당 단독 200/500 = 0.40으로 pass 구간. 그러나 합산 (200+205)/500 = 0.81 > 0.80 → fail. 비례 배점으로 30×(0.81-0.60)/0.40 = 16점. R1은 205/500 = 0.41로 비례 구간(0.60) 아래라 0점. 합 16 → 주의. R2-b가 없으면 0점으로 통과했을 입력.",
    ),
    (
    ...
    (
        ...
        "caution",
        GENERAL_CHECKS,
        "명세 R2는 집합건물 전제. 일반건물은 분모(호실 시세)를 구할 수 없어 unknown. R1·R8도 unknown. 담보 여력 3종이 모두 unknown이므로 §0-2에 따라 바닥 점수 15 → 주의. '판단 근거가 없다'를 '안전하다'로 표시하지 않는다.",
    ),
    (
    ...
    (
        ...
        "명세 R7: `peak >= 3` 이고 `비율 >= 30%`면 warn. 25 // 2 = 12점으로 단독으로는 안전 구간(0~14)을 못 벗어난다. 다만 담보 여력 3종이 모두 unknown이라 §0-2 바닥 점수 15가 적용돼 주의.",
    ),
    (
        ...
        "명세 R7은 건수와 비율을 모두 만족해야 warn. 2/5 = 40%로 비율은 넘지만 3건 미만이라 pass(0점). 등급이 주의인 것은 R7 때문이 아니라 담보 여력 3종이 모두 unknown이라 §0-2 바닥 점수 15가 적용되기 때문이다.",
    ),
    (
    ...
    (
        ...
        "명세 R8: (300+100+100)/600 = 0.833 > 0.80 → fail. §4-1에서 R8은 R2와 같은 §1 부등식이므로 가중치 30 → 위험(30~59). R1은 일반건물이라 unknown이며, 다가구에서 담보 여력을 잡는 규칙은 R8뿐이다.",
    ),
    (
    ...
    (
        ...
        "caution",
        GENERAL_CHECKS,
        "명세 R8: 선순위 보증금 미확보 → unknown. 선순위 보증금은 §2에서 오프라인 서류로만 얻을 수 있어 실제 제품에서 다가구는 거의 항상 이 경로를 탄다. 담보 여력 3종이 전부 unknown이라 §0-2 바닥 점수 15 → 주의. 이 바닥이 없으면 다가구 전체가 0점 안전으로 표시된다.",
    ),
]
