    ...
    <main>
      <section className="mx-auto grid w-full max-w-7xl gap-14 px-6 pb-16 pt-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-10 lg:pb-24 lg:pt-12">
        <div className="max-w-3xl">
          <div className="inline-flex rounded-full border border-coral/25 bg-white/80 px-4 py-2 text-sm text-slate-600 shadow-sm backdrop-blur">
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <Link
              to="/analysis/new"
              className="inline-flex items-center justify-center rounded-full bg-ink px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#0f523d]"
            >
              새 분석 시작하기
            </Link>
            <a
              href="#flow"
              className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-coral/40 hover:text-ink"
            >
              어떤 정보를 확인하나요
            </a>
          </div>
        ...
        <div className="relative">
          <div className="absolute -left-6 top-10 hidden h-24 w-24 rounded-full bg-coral/25 blur-2xl lg:block" />
      ...
      <section className="border-y border-coral/15 bg-white/70">
        <div className="mx-auto grid w-full max-w-7xl gap-6 px-6 py-14 lg:grid-cols-3 lg:px-10">
          {highlights.map((item) => (
            <article key={item.title} className="rounded-[1.75rem] border border-coral/15 bg-white p-6 shadow-sm">
      ...
      <section id="flow" className="mx-auto w-full max-w-7xl px-6 py-16 lg:px-10 lg:py-24">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {steps.map((step) => (
            <article key={step.number} className="bg-grid rounded-[2rem] border border-coral/15 bg-[size:28px_28px] p-6">
        ...
        <div className="mx-auto max-w-7xl rounded-[2.5rem] bg-ink px-8 py-10 text-white lg:px-12 lg:py-14">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            ...
            <div className="space-y-4 text-sm leading-7 text-slate-300">
              ...
              <Link
                to="/analysis/new"
                className="inline-flex rounded-full bg-white px-6 py-3 font-semibold text-ink transition hover:bg-coral/15"
              >
                분석 시작 페이지 보기
              </Link>
            </div>
